           ...
           SizedBox(
            ...
            child: ElevatedButton.icon(
              onPressed: () {
                ...
                Navigator.of(context).push(
                  MaterialPageRoute(
                    builder: (context) => PantallaReproductor(
                      pelicula: Map<String, dynamic>.from(pelicula as Map),
                    ),
                  ),
                );
              },
...
class _ModalComentariosContenidoState extends State<ModalComentariosContenido> {
  ...
  final String? uidActivo = usuarioActual?.id;

  if (uidActivo == null) {
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text("Error: No se detectó ninguna sesión activa de usuario."),
  ...
  try {
    ...
    if (perfilData == null) {
      throw Exception(
        "Tu cuenta no tiene un perfil registrado en la base de datos. "
        "Por favor, completa tu registro de perfil primero."
      );
    }
    ...
    if (_idComentarioEditando == null) {
      ...
      await supabase
          .from('comentarios')
          .update({'comentario': comentarioTexto})

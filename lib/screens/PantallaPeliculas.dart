...
class _PantallaPeliculasState extends State<PantallaPeliculas> {
  Future<List<dynamic>> _cargarPeliculasJson() async {
    final String respuesta = await DefaultAssetBundle.of(context).loadString('assets/json/peliculas.json');
  ...
  void _mostrarModalDetalles(BuildContext context, dynamic pelicula, String generos) {
    showModalBottomSheet(
      ...
      builder: (context) => ModalDetallesContenido(pelicula: pelicula, generos: generos),
    );
  }
...
class ModalDetallesContenido extends StatelessWidget {
  ...
  const ModalDetallesContenido({super.key, required this.pelicula, required this.generos});
...
class ModalComentariosContenido extends StatefulWidget {
  ...
  @override
  State<ModalComentariosContenido> createState() => _ModalComentariosContenidoState();
}
...
class _ModalComentariosContenidoState extends State<ModalComentariosContenido> {
  ...
  Future<void> _guardarOEditarComentario() async {
    ...
    if (uidActivo == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Error: No se detectó ninguna sesión activa de usuario."), backgroundColor: Colors.orange),
      );
      return;
    }
  ...
  Future<void> _eliminarComentario(dynamic idComentario) async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text("Error al eliminar: $e"), backgroundColor: Colors.red.shade900),
      );
    }
  }
  ...
  void _activarModoEdicion(dynamic idComentario, String textoComentario) {
    ...
    if (textoComentario.contains(': ')) {
      mensajeLimpio = textoComentario.substring(textoComentario.indexOf(': ') + 2);
    }
    setState(() {
        ...
        children: [
            ...
            child: StreamBuilder<List<Map<String, dynamic>>>(
              ...
              builder: (context, snapshot) {
                ...
                if (!snapshot.hasData || snapshot.data!.isEmpty) {
                  return const Center(
                    child: Text("Aún no hay opiniones.", style: TextStyle(color: Colors.white30, fontSize: 14)),
                  );
                }
                ...
                return ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final String creadorId = (item['id_usuario'] ?? '').toString().trim().toLowerCase();
                    final String comparadorUidLocal = (uidSesionMismoWidget ?? '').toString().trim().toLowerCase();
                    ...
                    final bool esMiComentario = comparadorUidLocal.isNotEmpty && creadorId == comparadorUidLocal;
                        ...
                        children: [
                          ...
                          if (esMiComentario) ...[
                            IconButton(
                              ...
                              onPressed: () => _activarModoEdicion(item['id'], textoCompleto),
                            ),
                            IconButton(
                              icon: const Icon(Icons.delete_outline_rounded, color: Colors.redAccent, size: 18),
                              onPressed: () => _mostrarDialogoConfirmacion(context, item['id']),
                            ),
                          ]
                        ],
                      ),
          ...
          Row(
            children: [
              ...
              IconButton(
                icon: _cargando
                    ? const SizedBox(width: 22, height: 22, child: CircularProgressIndicator(color: Colors.red, strokeWidth: 2))
                    : Icon(
                        _idComentarioEditando == null ? Icons.send_rounded : Icons.check_circle_outline_rounded, 
